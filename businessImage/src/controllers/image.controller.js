...
exports.batchDeleteByBusinessId = async (req, res) => {
  ...
  try {
    const result = await service.batchDeleteByBusinessId(businessId);
    return res.status(result.status).json(result.body);
  } catch (err) {
    logger.error(
      `CONTROLLER_ERROR | endpoint=batchDeleteByBusinessId | ${err.message}`,
    );
    return res
      .status(500)
...
exports.batchUpload = async (req, res) => {
  const { businessId, createdBy, uploadedBy } = req.body;
...
exports.getByBusinessId = async (req, res) => {
  try {
    const result = await service.getByBusinessId(req.params.businessId);
    return res.status(result.status).json(result.body);
  } catch (err) {
    return res
      .status(500)
