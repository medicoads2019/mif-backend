...
const BUSINESS_SERVICE_URL =
  process.env.BUSINESS_SERVICE_URL || BACKEND_BASE_URL;
...
async function batchUpload(businessId, createdBy, uploadedBy, files) {
  const start = Date.now();
  logger.info(
    `SERVICE_START | operation=batchUpload | businessId=${maskId(businessId)} | fileCount=${files ? files.length : 0}`,
  );
  ...
  for (const file of files) {
    ...
    try {
      ...
      const [original, thumb] = await Promise.all([
        uploadToCloudinary(file.buffer, { folder: "BusinessImageOriginal" }),
        uploadToCloudinary(file.buffer, {
          folder: "BusinessImageThumbnail",
          transformation: [
            {
              width: 600,
              height: 600,
      ...
      const count = await Image.countDocuments({ businessId });
      ...
      try {
        await axios.post(
          `${BUSINESS_SERVICE_URL}/businesss/${businessId}/images`,
          { imageId: saved._id.toString() },
        );
      } catch (ex) {
        logger.warn(
          `EXTERNAL_CALL_FAILED | service=BusinessService | imageId=${maskId(saved._id.toString())} | error=${ex.message}`,
        );
      }
...
async function getByBusinessId(businessId) {
  const images = await Image.find({ businessId }).sort({ orderIndex: 1 });
...
async function hardDeleteImage(id) {
  ...
  try {
    await axios.delete(
      `${BUSINESS_SERVICE_URL}/businesss/${entity.businessId}/images/${id}`,
    );
  } catch (ex) {
    logger.warn(
      `EXTERNAL_CALL_FAILED | service=BusinessService | imageId=${maskId(id)} | error=${ex.message}`,
    );
  }
  return {
    status: 200,
...
async function batchDeleteByBusinessId(businessId) {
  const start = Date.now();
  logger.info(
    `SERVICE_START | operation=batchDeleteByBusinessId | businessId=${maskId(businessId)}`,
  );

  const images = await Image.find({ businessId });

  // Return 202 immediately — background process does the actual deletion
  processBatchDeleteByBusinessId(images, businessId).catch((err) => {
    logger.error(
      `ASYNC_BATCH_DELETE_FAILED | businessId=${maskId(businessId)} | error=${err.message}`,
    );
  });

  logger.info(
    `SERVICE_COMPLETE | operation=batchDeleteByBusinessId | imageCount=${images.length} | durationMs=${Date.now() - start}`,
  );
  return {
    status: 202,
    body: {
...
async function processBatchDeleteByBusinessId(images, businessId) {
  logger.info(
    `ASYNC_BATCH_DELETE_START | businessId=${maskId(businessId)} | imageCount=${images.length}`,
  );
  ...
  logger.info(`ASYNC_BATCH_DELETE_COMPLETE | businessId=${maskId(businessId)}`);
}

module.exports = {
  batchDeleteByBusinessId,
  batchUpload,
  getById,
  getByStatus,
  getBySoftDelete,
