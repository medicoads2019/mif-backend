...
exports.batchUpload = async (req, res) => {
  const { businessFrameId, createdBy, uploadedBy } = req.body;
...
exports.getByBusinessFrameId = async (req, res) => {
  try {
    const result = await service.getByBusinessFrameId(
...
exports.batchDeleteByBusinessFrameId = async (req, res) => {
  const { businessFrameId } = req.params;
  if (!businessFrameId)
    return res
      .status(400)
      .json({ success: false, message: "businessFrameId is required" });
  try {
    const result = await service.batchDeleteByBusinessFrameId(businessFrameId);
    return res.status(result.status).json(result.body);
  } catch (err) {
    logger.error(
      `CONTROLLER_ERROR | endpoint=batchDeleteByBusinessFrameId | ${err.message}`,
    );
    return res
      .status(500)
      .json({ success: false, message: "Internal server error" });
